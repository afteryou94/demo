...
2. [**2. 실행 화면**](#2-실행-화면)
    * [1. 게시글 목록](#1-게시글-목록)
    * [2. 게시글 등록](#2-게시글-등록)
    * [3. 게시글 상세보기](#3-게시글-상세보기)
    * [4. 게시글 수정화면](#4-게시글-수정화면)
    * [5. 게시글 삭제화면](#5-게시글-삭제화면)
    * [6. 게시글 검색화면](#6-게시글-검색화면)
    * [1. 회원가입 화면](#1-회원가입-화면)
    * [2. 로그인 화면](#2-로그인-화면)
        * [2.1 OAuth 2.0 소셜 로그인 화면](#21-oauth-20-소셜-로그인-화면)
    * [3. 회원정보 수정](#3-회원정보-수정)
    * [1. 댓글 작성 화면](#1-댓글-작성-화면)
    * [2. 댓글 수정 화면](#2-댓글-수정-화면)
    * [3. 댓글 삭제 화면](#3-댓글-삭제-화면)
3. [**3. 구조 및 설계**](#3-구조-및-설계)
    * [1. 패키지 구조](#1-패키지-구조)
...
또한 회원 게시글, 댓글의 경우 사용자가 직접 로그인하는 경우를 생각하여 따로 실제 소유자인지 검증하는 부분을 생략하게 돼 아쉬웠고 추가적인 보안으로는 비회원은 BCrypt가 적용되지 않아 보안이 필요하다고 생각합니다.
